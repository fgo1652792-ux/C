...
module.exports = function(app, verifyToken) {
    ...
    const oauth2Client = new OAuth2Client(
        ...
        "https://c-production-89da.up.railway.app/auth/google/callback" 
    );
    ...
    app.get('/auth/google/callback', async (req, res) => {
        try {
            ...
            if (state && state.startsWith('exp://')) {
                ...
                res.redirect(`https://c-production-89da.up.railway.app/?token=${token}`);
            }
        } catch (error) {
            console.error('Auth error:', error);
            res.redirect('https://c-production-89da.up.railway.app/?auth_error=true');
        }
    });
